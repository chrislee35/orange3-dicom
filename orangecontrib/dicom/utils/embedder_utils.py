...
class ImageLoader:
    ...
    def load_image_or_none(self, file_path, frame, target_size=None):
        if file_path is None:
            return None
        image = self._load_image_from_url_or_local_path(file_path, frame)
    ...
    def load_image_bytes(self, file_path, frame, target_size=None):
        image = self.load_image_or_none(file_path, frame, target_size)
        if image is None:
            return None
    ...
    def _load_image_from_url_or_local_path(self, file_path: str, frame: int):
